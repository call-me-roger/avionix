...
7. **The pairing code input lives in `ConnectionForm`.** The spec assigns "clear the code after a failed attempt" to `MvpScreen` but lists no `code` prop, so `ConnectionForm` owns the text in local state and clears it in an effect when `pairing` falls from `true` to `false` while `state` is still `'pairing'` (a successful pair leaves `'pairing'`). _Execution note (final fix wave): the effect shape failed the `react-hooks/set-state-in-effect` lint rule, so the code, the in-flight flag and the buttons moved into a `PairingFields` component mounted only while `state === 'pairing'`; the `pairing` prop was dropped._
